...
def get_tensor_A(sampling_rate, year, select_diease_id):
    ...
    for y in range(2008, year + 1):
        if y < begin_year or y == year:
            ...
            file_name = "UCF_Obesity_begin_year_" + str(begin_year) \
...
def UCF_method(select_diease_id,sampling_rate, year):
    file_name = "UCF_Obesity_begin_year_" + str(begin_year) \
    ...
    df_csv = pd.DataFrame()
    file_name_csv = "UCF_Obesity_begin_year_" + str(begin_year) \
...
if __name__=='__main__':
    ...
    for year in range(begin_year,2018):
        UCF_method(select_diease_id, sampling_rate, year)
