...
def get_tensor_A(sampling_rate, year, select_diease_id):
    ...
    for y in range(2008, year + 1):
        if y < begin_year or y == year:
            ...
            file_name = "Spatial_mean_Obesity_begin_year_" + str(begin_year) \
...
def Spatial_mean(select_diease_id,sampling_rate,year):
    print(year)
    file_name = "Spatial_mean_Obesity_begin_year_" + str(begin_year) \
    ...
    A_result = [mean for i in range(624)]
    for i in ward_list:
    ...
    df_csv = pd.DataFrame()
    file_name_csv = "Spatial_mean_Obesity_begin_year_" + str(begin_year) \
...
if __name__=='__main__':
    ...
    for year in range(begin_year,2018):
        Spatial_mean(select_diease_id,sampling_rate,year)
